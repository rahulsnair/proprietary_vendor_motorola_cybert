...
kernel_so_list=("/vendor/lib/modules/$GKI_PATH/goodix_fod_mmi_u.ko" "/vendor/lib/modules/$GKI_PATH/ets_fod_mmi_u.ko")
kernel_so_name_list=("goodix_fod_mmi_u.ko" "ets_fod_mmi_u.ko")
...
function start_hal_service(){
    ...
    log "start ${hal_list[$1]} hal failed, index: ${1} vendor ${vendor_list[$1]}"
    setprop ctl.stop ${hal_list[$1]}
    sleep 0.4
    if [ "${vendor_list[$1]}" == "egis" ]; then
        log "will skip module remove for egis only to avoid kasan panic"
        # rmmod ${kernel_so_name_list[$1]}
    else
        log "start ${hal_list[$1]} hal failed, remove kernel so: ${kernel_so_name_list[$1]} index: ${1} vendor ${vendor_list[$1]}"
        rmmod ${kernel_so_name_list[$1]}
    fi
